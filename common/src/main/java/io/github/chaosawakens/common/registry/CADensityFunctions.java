...
public class CADensityFunctions { // Needed so that Vanilla datapacks don't directly affect CA dimension terrain gen (plus some other functions or optimizations, probably)
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_OFFSET = registerDensityFunction("mining_paradise/land/offset", b -> () -> DensityFunctions.flatCache(DensityFunctions.cache2d(DensityFunctions.lerp(DensityFunctions.blendAlpha(), DensityFunctions.add(DensityFunctions.constant(-0.3141592653D), DensityFunctions.spline(WorldGenUtil.miningParadiseOffset(new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, CONTINENTS)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, EROSION)), new DensityFunctions.Spline.Coordinate(getWrappedDensityFunction(b, RIDGES_FOLDED))))), DensityFunctions.blendOffset()))));
    ...
    public static final Supplier<ResourceKey<DensityFunction>> MINING_PARADISE_DEPTH = registerDensityFunction("mining_paradise/land/depth", b -> () -> DensityFunctions.add(DensityFunctions.yClampedGradient(-256, 480, 2.275D, -1.878D), getWrappedDensityFunctionHolder(b, MINING_PARADISE_OFFSET)));
